    ...
    def caption(self,image,vocab,max_length = 50):
        ...
        with torch.no_grad():
            ...
            for _ in range(max_length):
                hidden,states = self.lstm.lstm(x,states)
                output = self.lstm.fc1(hidden.squeeze(0))
                predicted = output.argmax(1)
                seq = [i.item() for i in predicted]
                x = self.lstm.embedding(predicted).unsqueeze(0)

                
        return [vocab.decoded[idx] for idx in seq]
...
def validate(model,dataloader,vocab):   
    ...
    with torch.no_grad():
        ...
        for idx,(images,captions,length) in enumerate(dataloader):
            pred = model.caption(images,vocab)
            print(pred)
            truth_caps = [decodetxt(txt, vocab) for txt in captions]
            all_pred_caps.extend(pred)
            all_truth_caps.extend(truth_caps)
            
            if idx == 0:
                plt.figure(figsize=(20, 10))
                for i in range(min(5, len(images))):  # Display up to 5 samples
                    # Convert image tensor to NumPy array and permute dimensions
                    image = images[i].permute(1, 2, 0).cpu().numpy()
                   
                    # If the image is normalized, denormalize it for display
                    if image.min() < 0 or image.max() > 1:  # Assuming normalization to [-1, 1] or [0, 1]
                    ...
                    # Set the title with predicted and ground truth captions
...
if not LOAD_MODEL:
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"[Epoch {epoch+1}/{EPOCHS}] Train Loss: {train_loss:.4f} Learning rate: {optimizer.param_groups[0]['lr']}")
